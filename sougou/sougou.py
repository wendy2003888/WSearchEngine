...
startPy = 0x1540;
#汉语词组表偏移
CNWordsOffset = 0x2628 # 词组列表的偏移地址
#全局拼音表
...
def deal(file_name):
    ...
    getPyTable(data[startPy:CNWordsOffset])
    getChinese(data[CNWordsOffset:])
...
if __name__ == '__main__':
    ...
    #保存结果  
    f = open('../app/sougou.txt','w')
    for word, count in GTable:
        #GTable保存着结果，是一个列表，每个元素是一个元组(词频,拼音,中文词组)，有需要的话可以保存成自己需要个格式
        #我没排序，所以结果是按照上面输入文件的顺序
        f.write( unicode( word + ' '  + str(count) ).encode('utf-8') )#最终保存文件的编码，可以自给改
        f.write('\n')
    f.close()    
